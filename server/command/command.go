...
type PluginAccessor interface {
	...
	// Remote ID access for shared channel invitations
	GetRemoteID() string
}
...
func (c *Handler) syncChannelMembersToMatrixRoom(channelID, roomID string) (int, int, error) {
	...
	for _, member := range allMembers {
		...
		if user.IsRemote() {
			...
			if err != nil {
				// If KV mapping doesn't exist, try to reconstruct Matrix user ID from Mattermost username
				originalMatrixUserID = c.reconstructMatrixUserIDFromUsername(user.Username)
				if originalMatrixUserID == "" {
					c.client.Log.Warn("Failed to get or reconstruct Matrix user ID for remote user", "error", err, "user_id", user.Id, "username", user.Username)
					continue
				}
				c.client.Log.Debug("Reconstructed Matrix user ID from username", "matrix_user_id", originalMatrixUserID, "mattermost_user_id", user.Id, "username", user.Username)
			}

...
func (b *MattermostToMatrixBridge) extractMattermostMentions(post *model.Post) *MattermostMentionResults {
	...
	b.API.LogDebug("Extracted mentions from Mattermost post", "post_id", post.Id, "user_mentions", results.UserMentions, "channel_mentions", results.ChannelMentions)
	return results
}
...
func (b *MattermostToMatrixBridge) addMatrixMentionsWithData(content map[string]any, post *model.Post, mentions *MattermostMentionResults) {
	...
	for _, username := range mentions.UserMentions {
		...
		if ghostUserID, exists := b.getGhostUser(user.Id); exists {
			...
			b.API.LogDebug("Converted Mattermost mention to Matrix", "username", username, "ghost_user_id", ghostUserID)
		} else {
			// Try to create ghost user for mention
			ghostUserID, err := b.CreateOrGetGhostUser(user.Id)
			if err != nil {
				b.API.LogDebug("Failed to create ghost user for mention", "username", username, "user_id", user.Id, "error", err)
				continue
			}
			...
			b.API.LogDebug("Created and converted Mattermost mention to Matrix", "username", username, "ghost_user_id", ghostUserID)
		}
	}
	...
	b.API.LogDebug("Added Matrix mentions to message", "post_id", post.Id, "mentioned_users", len(matrixUserIDs), "matrix_user_ids", matrixUserIDs)
}
